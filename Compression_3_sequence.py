...
def find_frequent_sequences(quantized_model, sequence_length=3, top_k=2**16):
...
def compress_model(quantized_model, compression_table, tree, sequence_length=3):
    compressed_files = []
    param_index = 0
    for param in quantized_model.parameters():
        weights = param.flatten().detach().cpu().numpy()  # Keep original dtype
...
def decompress_model_from_files(compressed_files, compression_table, sequence_length=3):
...
def main():
    ...
    if not compressed_files or not os.path.exists('compression_table.pkl'):
        ...
        compression_table = find_frequent_sequences(quantized_model, sequence_length=3)
        ...
        compressed_files = compress_model(quantized_model, compression_table, tree, sequence_length=3)
    ...
    decompressed_weights = decompress_model_from_files(compressed_files, compression_table, sequence_length=3)
